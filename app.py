...
if st.sidebar.button("🗑️ Clear Everything"):
    keys_to_keep = ["messages", "last_generated_image", "last_image_prompt", "uploaded_video_url", "edited_image_url", "t2v_video_url"]
    for key in keys_to_keep:
        if key in st.session_state:
            st.session_state[key] = [] if key == "messages" else None
    st.rerun()

st.sidebar.info("**Tips**\n\n• Type `/image ...` in chat for spicy images\n\n• Advanced Mode = fewer restrictions")
        ...
        if st.button("Turn Last Generated Image into Video"):
            ...
            with st.spinner("Creating video..."):
                try:
                    ...
                    num_frames = min(int(duration_sec * 16), 161)
                    if advanced_mode:
...
with tab_media:
    ...
    t2v_prompt = st.text_area("Describe your video", value="", key="t2v_prompt", height=100, placeholder="Describe the scene here...")
    col_a, col_b, col_c = st.columns(3)
    with col_a: res = st.selectbox("Resolution", ["480p", "720p"], index=1)
    with col_b: dur = st.selectbox("Duration", ["5s", "8s", "10s"], index=2)
    with col_c: asp = st.selectbox("Aspect ratio", ["16:9", "9:16", "1:1"], index=0)

    if st.button("Generate Video"):
        os.environ["FAL_KEY"] = fal_key
        with st.spinner("Creating video (this can take several minutes)..."):
            try:
                if advanced_mode:
                    num_frames = min(int(int(dur.replace("s","")) * 16), 161)
